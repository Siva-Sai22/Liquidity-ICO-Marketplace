...
export const CONTEXT_Provider = ({ children }) => {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            const tokenSaleBalance = await WOOX_TOKEN_CONTRACT.balanceOf(
                '0xBB67D8B175F375296D900213D5871d99A042a072'
            )
    ...
    const buyToken = async (nToken) => {
        try {
            ...
            const tokenPrice = 0.0001; // Price per token in Ether
            const price = tokenPrice * nToken;

            // Ensure the price does not exceed 18 decimal places
            const priceFormatted = price.toFixed(18);

            // Convert the formatted price to BigNumber
            const amount = ethers.utils.parseUnits(priceFormatted, 'ether');
    ...
    const transferNativeToken = async () => {
        try {
            ...
            const PROVIDER = await web3Provider()
            const signer = PROVIDER.getSigner()
            const TOKEN_SALE_ADDRESS = '0xBB67D8B175F375296D900213D5871d99A042a072'
